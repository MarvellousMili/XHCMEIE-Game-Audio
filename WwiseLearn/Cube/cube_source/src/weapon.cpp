...
{
	{ 1,	0,	0,	 250,  50, 0,   0,  1, "staff"		},
    { 1,	0,	0,	1400,  10, 0,   0, 20, "icegem"	},  // *SGRAYS
    { 1,	0,	0,	 100,  30, 0,   0,  7, "poisongem"	},
    { 1,	1,	1,	 800, 120, 80,  0, 10, "firegem"	},
    { 1,	0,	0,	1500, 100, 0,   0, 30, "lightninggem"  },
...
void selectgun(int a, int b, int c)
{
    ...
    if(s!=player1->gunselect) snd_clientevent(AK::EVENTS::SWITCHMAGIC);
    player1->gunselect = s;
...
void shoot(dynent *d, vec &targ)
{
	...
	{ 
		snd_clientevent(AK::EVENTS::NOMAGIC); 
    ...
    if(d->quadmillis && attacktime>200) snd_clientevent(AK::EVENTS::PUPMAGIC);
