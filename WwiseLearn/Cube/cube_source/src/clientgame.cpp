...
void arenarespawn()
{
    ...
    else if(arenadetectwait==0 || arenadetectwait<lastmillis)
    {
        ...
        if(dead>0 && (alive<=1 || (m_teammode && oneteam)))
        {
            ...
            if(alive) conoutf("Team %s is last wizard standing", lastteam);
            else conoutf("Everyone fainted!");
...
void selfdamage(int damage, int actor, dynent *act)
{
	...
	if(player1->health<=0)
    {
        if(actor==-2)
        {
            conoutf("You got knocked out by %s!", act->name);
        }
        else if(actor==-1)
        {
            actor = getclientnum();
        ...
        else
        {
            ...
            if(a)
            {
                ...
                {
                    conoutf("You got knocked out by a teammate (%s)", a->name);
                }
                else
                {
                    conoutf("You got knocked out by %s", a->name);
                };
            };
        };
        showscores(true);
        ...
        player1->pitch = 0;
        player1->roll = 60;
		snd_event(AK::EVENTS::DEFEATED_PLAYER);
        spawnstate(player1);
